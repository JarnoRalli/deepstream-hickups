...
To playback the stream, use one of the following:
    ...
    gst-launch-1.0 rtspsrc location=rtsp://127.0.0.1:8554/camera1 protocols=tcp latency=500 ! rtph264depay ! h264parse ! nvv4l2decoder ! queue ! nvvideoconvert ! queue ! mux.sink_1 nvstreammux name=mux width=1920 height=1080 batch-size=1 live-source=1 ! queue ! nvvideoconvert ! queue ! nvdsosd ! queue ! nveglglessink
...
class RTSPServer:
    ...
    def __init__(self, file_path: str) -> None:
        ...
        launch_str = f'filesrc location="{file_path}" ! decodebin name=decodebin ! queue ! videoconvert ! x264enc ! h264parse ! rtph264pay name=pay0 pt=96 config-interval=1'
        ...
        server.get_mount_points().add_factory("/camera1", factory)
        server.attach(None)
        print(f"RTSP server is running on rtsp://0.0.0.0:8554/camera1")
